...
Microsoft.Reporting.WebFormsClient._SessionKeepAlive.prototype =
{
    ...
    OnAppLoad: function()
    {
        ...
        if (this.KeepAliveIntervalSeconds != 0)
            this.TriggerWhenNecessary();
    },

    TriggerWhenNecessary: function () 
    {
        // If we have landed on this page as a result of a back button navigation (or when rendering 
        // from browser cache), we should immediatly attempt to keep the session alive. Of course 
        // chances are that the session has already expired.

        // Notes:
        // 1) If the cookie is found, we still delay the ping by 1 seconds to esnure that all other
        //    on-load activities are completed first (some on-load event handlers, do a thread switch 
        //    using setTimeout(..., 0) and We want those to have a higher priority than the ping here)
        // 2) When a report with the same KeepAliveUrl is accessed from the same browser session, 
        //    we assume that a ping is required (a false-positive case). This can happen very rarely.
        // 3) Because no expiration date is set on this cookie, it expires as soon as the browser 
        //    is closed. (This is the desired behavior)

        var uniqueCookie = escape(this.KeepAliveUrl) + "_SKA";
        var timeout = 1000; // 1 second
        if (document.cookie.indexOf(uniqueCookie) == -1)
        {
            document.cookie = uniqueCookie + "=1";
            timeout = this.KeepAliveIntervalSeconds * 1000;
        }
        this.m_keepAliveTimerId = setTimeout(this.m_onTimerIntervalDelegate, timeout);
    },
    ...
    OnKeepAliveRequestCompleted: function(executor, eventArgs)
    {
        ...
        {
            this.m_keepAliveTimerId = setTimeout(this.m_onTimerIntervalDelegate, this.KeepAliveIntervalSeconds * 1000);
        }
    }
}
